<div class="py-2 animate-fade-in-up max-w-4xl mx-auto">
    ...
    <div class="space-y-6">
        @forelse($messages as $msg)
            <div class="group bg-white p-6 rounded-2xl shadow-[0_4px_20px_rgb(230,217,184,0.2)] border border-[#E6D9B8]/60 transition-all hover:shadow-[0_8px_30px_rgb(184,151,96,0.15)]">
                
                {{-- Header Pesan --}}
                <div class="flex justify-between items-start mb-4">
                    <div class="flex items-center gap-4">
                        {{-- Avatar --}}
                        <div class="w-12 h-12 rounded-full bg-[#F2ECDC] border border-[#E6D9B8] flex items-center justify-center text-[#9A7D4C] font-serif font-bold text-lg shadow-sm">
                            {{ substr($msg->sender_name, 0, 1) }}
                        </div>
                        
                        <div>
                            <h4 class="font-serif font-bold text-lg text-[#5E4926] leading-tight">{{ $msg->sender_name }}</h4>
                ...
                {{-- Isi Pesan --}}
                <div class="pl-16">
                    <div class="text-[#5E4926] text-sm leading-relaxed mb-6 bg-[#F9F7F2]/50 p-4 rounded-xl border border-[#F2ECDC] italic relative">
                        <i class="fa-solid fa-quote-left absolute -top-2 -left-2 text-[#E6D9B8] text-xl"></i>
                        {{ $msg->content }}
                    </div>

                    {{-- List Balasan --}}
                    <div class="space-y-4 relative before:absolute before:left-0 before:top-0 before:bottom-0 before:w-0.5 before:bg-[#E6D9B8]/50 before:rounded-full pl-6">
                            ...
                            <div class="bg-[#F9F7F2] p-4 rounded-xl border border-[#E6D9B8] relative">
                                ...
                                <div class="flex justify-between items-start mb-2">
                                    <div class="flex items-center gap-2">
                                        <span class="text-[10px] font-bold bg-[#B89760] text-white px-2 py-0.5 rounded-full border border-[#9A7D4C] shadow-sm flex items-center gap-1">
                                            <i class="fa-solid fa-crown text-[8px]"></i> Mempelai
                                        </span>
                                        <span class="text-[10px] text-[#9A7D4C]">{{ $reply->created_at->diffForHumans() }}</span>
                                    </div>
                                    <button wire:click="delete({{ $reply->id }})" class="text-[#C6AC80] hover:text-red-500 text-xs transition">
                        ...
                        <div class="mt-4">
                            @if($replyingToId === $msg->id)
                                <div class="animate-fade-in-down bg-white p-4 rounded-xl border border-[#B89760] shadow-md relative z-10">
                                    <textarea wire:model="replyContent" 
                                              ...
                                              rows="3" 
                                              placeholder="Tulis balasan ucapan terima kasih..."></textarea>
                                    
                                    <div class="flex gap-2 justify-end">
                                        <button wire:click="cancelReply" class="text-xs font-bold text-[#7C6339] hover:text-[#5E4926] px-3 py-2 rounded transition">Batal</button>
                            ...
                            @else
                                <button wire:click="setReply({{ $msg->id }})" 
                                    ...
                                    <div class="w-6 h-6 rounded-full bg-[#F2ECDC] flex items-center justify-center group-hover/btn:bg-[#E6D9B8] transition">
        ...
        @empty
            ...
            <div class="py-16 text-center bg-white rounded-2xl border-2 border-dashed border-[#E6D9B8]">
                <div class="w-16 h-16 bg-[#F9F7F2] rounded-full flex items-center justify-center mx-auto mb-4 text-[#B89760]">
