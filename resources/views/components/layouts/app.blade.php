...
<body class="font-sans h-svh flex antialiased bg-[#F9F7F2] text-[#5E4926]" x-data="{ sidebarOpen: false }"
   ...
   {{ request()->routeIs('admin.transactions') ? 'bg-white shadow-sm border border-[#E6D9B8] text-[#9A7D4C]' : 'text-[#7C6339] hover:bg-white/60 hover:text-[#9A7D4C]' }}">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="overflow-hidden min-w-0">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="text-xs text-[#9A7D4C] hover:text-[#B89760] transition flex items-center gap-1">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-y-scroll h-svh bg-[#F9F7F2]">
        ...
        <!-- TOP HEADER -->
        <header
            class="h-20 flex items-center justify-between px-6 lg:px-10 bg-[#F9F7F2]/80 backdrop-blur-md sticky top-0 z-20 border-b border-[#E6D9B8]/30 shrink-0">
            <div class="flex items-center gap-4">
                <!-- Hamburger Button (Mobile Only) -->
                <button @click="sidebarOpen = true"
                    class="lg:hidden text-[#9A7D4C] hover:text-[#B89760] focus:outline-none transition p-1">
