...
@slot('head')
    ...
    <link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@400;600;800&family=Great+Vibes&family=Raleway:wght@300;400;600&display=swap" rel="stylesheet">
@endslot
...
<style>
    ...
    ::-webkit-scrollbar-thumb:hover { background: #8E6E34; }
...
{{-- ======================================================================= --}}
<div id="loading-overlay" class="fixed inset-0 z-[9999] bg-[#050505] flex flex-col items-center justify-center transition-opacity duration-1000">
    ...
    <div class="w-48 h-[1px] bg-[#333] relative overflow-hidden">
        <div id="gold-line" class="absolute top-0 left-0 h-full bg-gradient-to-r from-[#bf953f] to-[#fcf6ba] w-0 transition-all duration-[2000ms] ease-out"></div>
    </div>
...
<div class="relative bg-noise text-gray-300 selection:bg-[#8E6E34] selection:text-black">
    ...
    @if (!empty($theme['music_url']))
        <div x-data="youtubePlayer('{{ $theme['music_url'] }}')" x-init="initPlayer()" @play-music.window="playMusic()"
            ...
            <button @click="togglePlay" class="group relative w-12 h-12 flex items-center justify-center">
                ...
                <!-- Icon -->
                <i class="fa-solid fa-music absolute -top-4 -right-4 text-[10px] text-[#8E6E34] opacity-0 group-hover:opacity-100 transition-opacity"></i>
            </button>
    ...
    {{-- HERO SECTION --}}
    <section class="relative h-screen w-full overflow-hidden flex flex-col justify-center items-center text-center px-6">
        <!-- Parallax Background -->
        <div class="absolute inset-0 z-0">
            <img src="{{ $coverImage }}" class="w-full h-full object-cover opacity-40 scale-105 animate-[pulse_10s_ease-in-out_infinite]" style="animation-duration: 20s;">
        ...
        <div class="relative z-10 space-y-6" data-anim="fade-up" data-duration="1.5s">
            ...
            <div class="flex items-center justify-center gap-4 text-[#F4E2AB]/80 font-main text-sm md:text-base tracking-widest">
    ...
    <section class="py-24 px-6 bg-[#0a0a0a] relative">
        <div class="absolute top-0 left-1/2 -translate-x-1/2 w-[1px] h-20 bg-gradient-to-b from-transparent to-[#8E6E34]"></div>
    ...
    <section class="py-20 bg-[#050505]">
        <div class="max-w-6xl mx-auto px-6">
            <div class="grid md:grid-cols-2 gap-16 items-center">
                ...
                <div class="relative group order-1 md:order-2 mx-auto md:mx-0 w-64 h-80 md:w-80 md:h-[450px]" data-anim="zoom-in">
                    <div class="absolute -inset-2 border border-[#8E6E34]/30 rounded-t-[100px] rounded-b-[0px] group-hover:scale-105 transition-transform duration-700"></div>
                    <div class="w-full h-full overflow-hidden rounded-t-[100px] relative">
                        <img src="{{ $groomImage }}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-1000">
                ...
                <div class="relative group order-3 mx-auto md:mx-0 w-64 h-80 md:w-80 md:h-[450px]" data-anim="zoom-in">
                    <div class="absolute -inset-2 border border-[#8E6E34]/30 rounded-t-[100px] rounded-b-[0px] group-hover:scale-105 transition-transform duration-700"></div>
                    <div class="w-full h-full overflow-hidden rounded-t-[100px] relative">
                        <img src="{{ $brideImage }}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-1000">
                        <div class="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-80"></div>
                    </div>
                </div>
    ...
    <section class="py-24 relative bg-cover bg-fixed bg-center" style="background-image: url('{{ $coverImage }}');">
        <!-- Overlay -->
        ...
        <div class="relative z-10 max-w-4xl mx-auto px-6 text-center text-white">
            ...
            <div class="border border-[#8E6E34]/40 bg-[#0a0a0a]/80 p-8 md:p-12 relative" data-anim="zoom-in">
                ...
                <h3 class="font-main text-3xl md:text-4xl uppercase tracking-widest mb-4">{{ $invitation->event_data[0]['title'] }}</h3>
            ...
            <div class="columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4 max-w-7xl mx-auto">
                @foreach ($moments as $index => $photo)
                    <div class="break-inside-avoid relative group overflow-hidden cursor-pointer">
                        <div class="absolute inset-0 border-[1px] border-[#F4E2AB] opacity-0 scale-95 group-hover:scale-100 group-hover:opacity-100 transition-all duration-500 z-10"></div>
                        <img src="{{ asset($photo) }}" class="w-full grayscale brightness-75 group-hover:grayscale-0 group-hover:brightness-100 transition-all duration-700">
                    </div>
                @endforeach
            </div>
        </section>
    ...
    @if (!empty($gifts))
        <section class="py-24 px-6 bg-[#050505]">
            <div class="max-w-4xl mx-auto text-center">
                ...
                <p class="font-body text-gray-400 mb-12 max-w-lg mx-auto">Your presence is the greatest gift. However, if you wish to honor us with a gift, a digital envelope is available.</p>
                ...
                <div class="flex flex-wrap justify-center gap-8">
                    @foreach ($gifts as $gift)
                        ...
                        <div class="group relative w-full max-w-[360px] h-[220px] rounded-xl bg-gradient-to-br from-[#1a1a1a] to-[#000] border border-[#333] shadow-2xl p-6 flex flex-col justify-between text-left hover:-translate-y-2 transition-transform duration-500"
                            ...
                            <div class="absolute inset-0 rounded-xl border border-[#8E6E34] opacity-0 group-hover:opacity-30 transition duration-500"></div>

                            <!-- Top: Chip & Bank -->
                            <div class="flex justify-between items-start">
                                <div class="w-10 h-8 bg-gradient-to-br from-[#F4E2AB] to-[#8E6E34] rounded-md relative overflow-hidden">
                                    <div class="absolute top-1/2 w-full h-[1px] bg-black/20"></div>
                                    <div class="absolute left-1/2 h-full w-[1px] bg-black/20"></div>
                                </div>
                                <span class="font-main font-bold text-gray-400 tracking-wider text-sm">{{ strtoupper($gift['bank_name']) }}</span>
                            </div>
                            ...
                            <div class="flex justify-between items-end">
                                <div>
                                    <p class="text-[0.6rem] text-[#8E6E34] uppercase tracking-widest mb-1">Card Holder</p>
                                    <p class="font-body text-sm text-gray-300 uppercase tracking-wider">{{ $gift['account_name'] }}</p>
                                </div>
                                <i class="fa-regular fa-clone text-[#8E6E34] cursor-pointer hover:text-white transition" onclick="navigator.clipboard.writeText('{{ $gift['account_number'] }}')"></i>
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    <section class="py-24 px-6 bg-[#0a0a0a]">
        <div class="max-w-2xl mx-auto border border-[#8E6E34]/20 p-8 md:p-12 bg-[#050505]" data-anim="fade-up">
            ...
            <div class="font-body text-gray-300 
...
<script>
    function countdown(eventDate) {
        return {
            ...
            now: Date.now(),
            timer: null,
            start() { this.timer = setInterval(() => { this.now = Date.now() }, 1000) },
